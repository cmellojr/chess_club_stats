"""SQLite-backed HTTP response cache for providers.
...
class CachedResponse:
    ...
    Implements only the subset used by client providers: ``status_code``,
    ``json()``, and ``raise_for_status()``. Reports the original HTTP
    status code so that cached 404 responses are handled identically to live
    ones.
