...
class CachedResponse:
    ...
    Implements only the subset used by
    :class:`~chessclub.providers.chesscom.client.ChessComClient`:
    ``status_code``, ``json()``, and ``raise_for_status()``.  Reports
    the original HTTP status code so that cached 404 responses are
    handled identically to live ones.
